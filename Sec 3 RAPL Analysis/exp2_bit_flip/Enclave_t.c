...
static sgx_status_t SGX_CDECL sgx_ecall_rsa_benchmark(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_rsa_benchmark_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_rsa_benchmark_t* ms = SGX_CAST(ms_ecall_rsa_benchmark_t*, pms);
	ms_ecall_rsa_benchmark_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_rsa_benchmark_t), ms, sizeof(ms_ecall_rsa_benchmark_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	_in_retval = ecall_rsa_benchmark((const unsigned char*)_in_msg, _tmp_msg_len, _in_sig, _tmp_sig_len, __in_ms.ms_iterations);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
SGX_EXTERNC const struct {
	...
	3,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_rsa_benchmark, 0, 0},
