...
# --- 1. RSA PARAMETERS AUTO-EXTRACTION ---
def get_params(filename):
    ...
    # Prefix: Windows 1-4 (Bits 1-12)
    prefix = d_bin[:12]
    # Target: Window 5 (Bits 13-15)
    target = d_bin[12:15]
    
    return n, prefix, target

N, KNOWN_PREFIX, REAL_BITS_W5 = get_params('key.h')

# --- 2. LEAKAGE MODEL ---
def get_hw(n):
    return bin(n).count('1')

def model_window_5(msg_hex, guess_int, modulus, prefix_str):
    msg_int = int(msg_hex, 16)
    R = pow(2, 512, modulus) # Montgomery constant for 512-bit key
    
    # Construct the 15-bit exponent hypothesis (12 bits prefix + 3 bits guess)
    full_guess_str = prefix_str + format(guess_int, '03b')
    ...
    # Convert to Montgomery form (the internal CPU register state)
...
print(f"[+] Attacking Window 5 using: {latest_dir}")
print(f"[+] Traces: {len(df)}")
print(f"[+] Auto-extracted Prefix (W1-W4): {KNOWN_PREFIX}")
...
print(f"[+] Data Cleaning: Removed {original_count - len(df)} outliers.")

# --- 4. CORRELATION LOOP ---
results = []
for guess in range(8):
    guess_str = format(guess, '03b')
    hws = df['msg_hex'].apply(lambda x: model_window_5(x, guess, N, KNOWN_PREFIX))
    r, p = pearsonr(hws, df['energy_uj'])
    results.append({'guess': guess_str, 'r': r, 'abs_r': abs(r), 'p': p})

# --- 5. RESULTS & RANKING ---
# Rule: Highest |r| where r < 0 (Negative Correlation)
...
print("\n================= WINDOW 5 ATTACK REPORT =================")
print(f"Attacker's Best Guess: {winner['guess']} (r={winner['r']:.6f})")
print(f"Actual Key Bits:       {REAL_BITS_W5}")
print("----------------------------------------------------------")

if winner['guess'] == REAL_BITS_W5:
    print("RESULT: SUCCESS! Window 5 recovered.")
else:
    # Find rank of correct bits
    rank = [r['guess'] for r in sorted_all].index(REAL_BITS_W5) + 1
